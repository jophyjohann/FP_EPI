	...
	def main(self):
	  ...
	  ### Define some fitting functions
		...
		for param in fit_param[0]:
			...
			print("{} = {:.6} \t±{:.6f}  \t[±{:.4}] \t({:.4}...{:.4f})".format(param,popt[1][i],np.sqrt(np.diag(pcov[1]))[i],np.abs(popt[2][i] - popt[0][i]) + np.sqrt(np.diag(pcov[0]))[i] + np.sqrt(np.diag(pcov[2]))[i],min(popt[0][i],popt[2][i]) - np.sqrt(np.diag(pcov[0]))[i], max(popt[0][i],popt[2][i]) + np.sqrt(np.diag(pcov[2]))[i]))
